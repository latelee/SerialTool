...
BOOL CSCOMMDlg::OnInitDialog()
{
    ...
    m_Speed.AddString("300");
    m_Speed.AddString("600");
    m_Speed.AddString("1200");
    m_Speed.AddString("2400");
    m_Speed.AddString("4800");
    m_Speed.AddString("9600");
    m_Speed.AddString("19200");
    m_Speed.AddString("38400");
    m_Speed.AddString("43000");
    m_Speed.AddString("56000");
    m_Speed.AddString("57600");
    m_Speed.AddString("74880");
...
#if 0
    CString strSpeed;
    m_Speed.GetLBText(5, strSpeed);

    int sspeed = _ttoi(strSpeed);
#endif
...
void CSCOMMDlg::OnButtonOpenport() 
{
	...
	else  //打开串口
	{
		...
		if (m_Port.InitPort(this, m_nCom, m_nBaud,m_cParity,m_nDatabits,m_nStopbits,m_dwCommEvents,512))
		{
			m_Port.StartMonitoring();
			m_ctrlIconOpenoff.SetIcon(m_hIconRed);
			strStatus.Format("STATUS：COM%dOPENED，-%d,%c,%d,%d",m_nCom, m_nBaud,m_cParity,m_nDatabits,m_nStopbits);
			//"当前状态：串口打开，无奇偶校验，8数据位，1停止位");
		}
		else
		{
...
void CSCOMMDlg::OnSelendokComboSpeed() 
{
    ...
    m_Speed.GetLBText(i, temp);
    m_nBaud = _ttoi(temp);

    //m_Speed.GetItemData(IDC_COMBO_SPEED);
    m_Speed.GetWindowTextA(temp);
